...
void test_resetInterruptFlag_when_not_initialized()
{
    ...
    TEST_ASSERT_FALSE_MESSAGE(MCP2515Module.resetInterruptFlag(0), "Resetting Interrupt-Flag successfull.");
    ...
    if ((MCP2515Module.getLastMCPError() & static_cast<uint16_t>(MCP2515Error::MAIN_NOT_INITIALIZED))!=static_cast<uint16_t>(MCP2515Error::MAIN_NOT_INITIALIZED))
    {
        TEST_ASSERT_EQUAL_HEX16_MESSAGE(static_cast<uint16_t>(MCP2515Error::MAIN_NOT_INITIALIZED),
                                        ...
                                        "Wrong Error-Code for resetting Interrupt-Flag when not initialized.");
    }
}

void test_resetInterruptFlag_when_initialized()
{
    TEST_ASSERT_TRUE_MESSAGE(MCP2515Module.getIsInitialized(), "Module is not initialized.");

    TEST_ASSERT_TRUE_MESSAGE(MCP2515Module.resetInterruptFlag(0), "Resetting Interrupt-Flag not successfull.");
}

void test_resetInterruptFlag_for_defined_flags()
{
    TEST_ASSERT_TRUE_MESSAGE(MCP2515Module.getIsInitialized(), "Module is not initialized.");
...
void test_resetInterruptFlag_for_undefined_flags()
{
    TEST_ASSERT_TRUE_MESSAGE(MCP2515Module.getIsInitialized(), "Module is not initialized.");

    for (size_t i=8; i<11; i++)
    {
        TEST_ASSERT_FALSE_MESSAGE(MCP2515Module.resetInterruptFlag(i), "Resetting Interrupt-Flag successfull.");
        if ((MCP2515Module.getLastMCPError() & static_cast<uint16_t>(MCP2515Error::MAIN_VALUE_OUTA_RANGE))!=static_cast<uint16_t>(MCP2515Error::MAIN_VALUE_OUTA_RANGE))
        {
            TEST_ASSERT_EQUAL_HEX16_MESSAGE(static_cast<uint16_t>(MCP2515Error::MAIN_VALUE_OUTA_RANGE),
                                            MCP2515Module.getLastMCPError(),
                                            "Wrong Error-Code for resetting undefined Interrupt-Flag.");
        }
    }
}

void test_resetInterruptFlag_for_each_flag()
{
    TEST_ASSERT_TRUE_MESSAGE(MCP2515Module.getIsInitialized(), "Module is not initialized.");
...
int runUnityTests() {
    ...
    RUN_TEST(test_resetInterruptFlag_when_not_initialized);
    RUN_TEST(test_resetInterruptFlag_when_initialized);
    RUN_TEST(test_resetInterruptFlag_for_defined_flags);
    RUN_TEST(test_resetInterruptFlag_for_undefined_flags);
    RUN_TEST(test_resetInterruptFlag_for_each_flag);
