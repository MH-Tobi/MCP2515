...
void test_getRtrFromReceiveBuffer_when_not_initialized()
{
    ...
    MCP2515Module.getRtrFromReceiveBuffer(0);
    ...
    if ((MCP2515Module.getLastMCPError() & static_cast<uint16_t>(MCP2515Error::MAIN_NOT_INITIALIZED))!=static_cast<uint16_t>(MCP2515Error::MAIN_NOT_INITIALIZED))
    {
        TEST_ASSERT_EQUAL_HEX16_MESSAGE(static_cast<uint16_t>(MCP2515Error::MAIN_NOT_INITIALIZED),
                                        ...
                                        "Wrong Error-Code for get RTR from ReceiveBuffer when not initialized.");
    }
}

void test_getRtrFromReceiveBuffer_when_initialized()
{
    ...
    TEST_ASSERT_EQUAL_HEX16_MESSAGE(static_cast<uint16_t>(MCP2515Error::NO_ERROR),
                                    MCP2515Module.getLastMCPError(),
                                    "Error-Code occured for get RTR from ReceiveBuffer when initialized.");
}

void test_getRtrFromReceiveBuffer_from_defined_buffer()
{
    TEST_ASSERT_TRUE_MESSAGE(MCP2515Module.getIsInitialized(), "Module is not initialized.");

    for (size_t i=0; i<2; i++)
    {
        MCP2515Module.getRtrFromReceiveBuffer(i);
        TEST_ASSERT_EQUAL_HEX16_MESSAGE(static_cast<uint16_t>(MCP2515Error::NO_ERROR),
                                        MCP2515Module.getLastMCPError(),
                                        "Error-Code occured for get RTR from defined ReceiveBuffer.");
    }
}

void test_getRtrFromReceiveBuffer_from_undefined_buffer()
{
    TEST_ASSERT_TRUE_MESSAGE(MCP2515Module.getIsInitialized(), "Module is not initialized.");
    ...
    for (size_t i=2; i<4; i++)
    {
        ...
        if ((MCP2515Module.getLastMCPError() & static_cast<uint16_t>(MCP2515Error::MAIN_VALUE_OUTA_RANGE))!=static_cast<uint16_t>(MCP2515Error::MAIN_VALUE_OUTA_RANGE))
        {
            TEST_ASSERT_EQUAL_HEX16_MESSAGE(static_cast<uint16_t>(MCP2515Error::MAIN_VALUE_OUTA_RANGE),
                                            MCP2515Module.getLastMCPError(),
                                            "Wrong Error-Code for get RTR from undefined ReceivedBuffer.");
        }
    }
}

void test_getRtrFromReceiveBuffer_from_received_non_rtr_message()
{
    TEST_ASSERT_TRUE_MESSAGE(MCP2515Module.getIsInitialized(), "Module is not initialized.");
    ...
    if ((MCP2515Module.check4InterruptFlags() & 0x01) == 0x01)
    {
        TEST_ASSERT_EQUAL_UINT32_MESSAGE(0,
                                        MCP2515Module.getRtrFromReceiveBuffer(0),
                                        "Get wrong RTR-Value from ReceiveBuffer 0.");
    }else{
        TEST_ASSERT_EQUAL_UINT32_MESSAGE(0,
                                        MCP2515Module.getRtrFromReceiveBuffer(1),
                                        "Get wrong RTR-Value from ReceiveBuffer 1.");
    }
}

void test_getRtrFromReceiveBuffer_from_received_rtr_message()
{
    TEST_ASSERT_TRUE_MESSAGE(MCP2515Module.getIsInitialized(), "Module is not initialized.");

    TEST_ASSERT_TRUE_MESSAGE(MCP2515Module.setLoopbackMode(), "Loopback Mode is not set.");
    delay(100);
    TEST_ASSERT_TRUE_MESSAGE(MCP2515Module.fillTransmitBuffer(0, 1, false, true, 4, 0), "Filling the TransmitBuffer 0 not successfull.");
    delay(100);
    TEST_ASSERT_TRUE_MESSAGE(MCP2515Module.sendMessage(0, 0), "Sending Message not successfull.");
    delay(500);
...
int runUnityTests() {
    ...
    RUN_TEST(test_getRtrFromReceiveBuffer_when_not_initialized);
    RUN_TEST(test_getRtrFromReceiveBuffer_when_initialized);
    RUN_TEST(test_getRtrFromReceiveBuffer_from_defined_buffer);
    RUN_TEST(test_getRtrFromReceiveBuffer_from_undefined_buffer);
    RUN_TEST(test_getRtrFromReceiveBuffer_from_received_non_rtr_message);
    RUN_TEST(test_getRtrFromReceiveBuffer_from_received_rtr_message);
